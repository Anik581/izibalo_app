jQuery(function(){
  ...
  $("#task_max_work_time").change(function () {                    
	...
	$(document).ready(function() {
  	...
  	var move = ($( ".target" ).offset().left + (2 * space_on_the_right_of_trget) - space_without_calendar) - parseInt($("#visible_calendar").css("padding-left"));
 		var validate_move = 0
		if (move - visible_calendar_width > 0) {
		  if ((move - visible_calendar_width) < (hidden_calendar_width - visible_calendar_width)) {
		    ...
		    validate_move = (move - space_on_the_right_of_trget) - visible_calendar_width;
		  }
		}
